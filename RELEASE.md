...
2. On every push to `main`, the `Release Please` workflow runs CI (ansible-lint, CodeQL), then [release-please](https://github.com/googleapis/release-please) opens or updates a release PR that:
   - bumps `version.rb` and the `version:` in `provisioner.yml`
...
4. The `Build Provisioner Artifact` workflow then checks out the tag (submodules included), fetches the sha256-verified core driver pinned in `driver.version`, and uploads four assets to the release:
   - `hcl_domino_additional_provisioner-<version>.tar.gz` + `.sha256` — the immutable, registry-shaped archive the provisioner catalog records
   - `hcl_domino_additional_provisioner.tar.gz` + `.sha256` — a version-less copy at a stable URL
...
Run the `Build Provisioner Artifact` workflow via manual dispatch with the release's `version` (for example `0.1.12`) and its `tag_name`.
