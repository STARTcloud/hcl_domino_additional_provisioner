...
1. Go to the [GitHub Security Advisory page](https://github.com/STARTcloud/hcl_domino_additional_provisioner/security/advisories)
...
HCL Domino Additional Provisioner is an Ansible-based provisioner package driven by the core_provisioner Vagrant driver. The security-relevant areas are:
...
- **Secrets handling** — installer download credentials and Domino passwords belong in `.secrets.yml`, which is gitignored; never commit them to `Hosts.yml` or the repository. Domino ID files under `id-files/` are likewise gitignored — never commit them.
